...
def greedy_decode(model,source,source_mask,tokenizer_src,tokenizer_tgt,max_len,device):
    ...
    while True:
        ...
        # (B, seq_len, d_model) -> (B, 1, 1, seq_len)
        decoder_mask = causal_mask(decoder_input.size(1)).type_as(source_mask).to(device)

        # Calculate the output for the decoder
...
def run_validation(model,validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_messages,global_step,writer,num_examples=2):
    model.eval()
    count = 0
    source_texts = []
    expected_texts = []
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            print_messages('-'*console_width)
            print_messages(f"{'SOURCE: ':>12}{source_text}")
            print_messages(f"{'TARGET: ':>12}{target_text}")
            print_messages(f"{'PREDICTED: ':>12}{model_output_text}")
...
def get_ds(config):
    ...
    train_ds = BilingualDataset(train_ds_raw,src_tokenizer, tgt_tokenizer, config['seq_len'], config['lang_src'], config['lang_tgt'])
    val_ds = BilingualDataset(val_ds_raw,src_tokenizer, tgt_tokenizer, config['seq_len'], config['lang_src'], config['lang_tgt'])
...
def train_model(config):
    ...
    for epoch in range(initial_epoch,config['num_epochs']):
        model.train()
        batch_iterator = tqdm(train_dataloader, desc=f'Processing epoch {epoch:02d}', unit='batch')
        for batch in batch_iterator:
            ...
            projection_output = model.project(decoder_output) # (B, seq_len, vocab_tgt_len)

            label = batch['label'].to(device) # (B,seq_len)
            # (B, seq_len, vocab_len) -> (B*seq_len, vocab_len)
