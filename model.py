...
class PostionalEncoding(nn.Module):
    def __init__(self,d_model:int,seq_len, dropout:float)->None:
        ...
        deno_term = torch.exp(torch.arange(0,d_model,2).float()*(-math.log(10000.0)/d_model))
        # Apply sine to even indices, cosine to odd indices
        pe[:,0::2] =torch.sin(position*deno_term)
        pe[:,1::2] =torch.cos(position*deno_term)
        pe = pe.unsqueeze(0)

        # saving the positional encoding as a buffer
        # it means it will not be updated by the optimizer 
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self,q,k,v,mask):
        query = self.w_q(q) # (Batch, seq_len, d_model)->(B,Seq_len,d_model)
        ...
        # (Batch, h, seq_len,d_model)->(Batch, seq_len,h,d_k)->(Batch,seq_len,d_model)
...
class Transformer(nn.Module):
    ...
    src_pos:PostionalEncoding,tgt_pos:PostionalEncoding,projection_layer:ProjectionLayer)->None:
        super().__init__()
...
tgt_seq_len:int,d_model:int=512,h:int=8,dropout:float=0.1,N:int=6,d_ff:int=2048)->Transformer:
